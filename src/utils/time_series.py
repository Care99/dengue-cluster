...
def get_ts(year:str, week:str, department:str)->list[float]:
    start_date = datetime.strptime(start_date_index[int(year.split('-')[0])-2019],'%m/%d/%Y') + timedelta(weeks=int(week))
    end_date = start_date + timedelta(weeks=11)
    return get_time_series_from_range(start_date,end_date,department)

def get_historical_data(department:str)->list[float]:
    start_date = datetime.strptime("10/05/2019",'%m/%d/%Y')
    end_date = datetime.strptime("09/24/2022",'%m/%d/%Y')
    return get_time_series_from_range(start_date,end_date,department)

def get_2022_2023_data(department:str)->list[float]:
    start_date = datetime.strptime("10/01/2022",'%m/%d/%Y')
    end_date = datetime.strptime("09/30/2023",'%m/%d/%Y')
    return get_time_series_from_range(start_date,end_date,department)
...
def get_historical_data_window(
    ...
    start_date = datetime.strptime("10/05/2019",'%m/%d/%Y') + timedelta(weeks=week_window)
    end_date = datetime.strptime("09/24/2022",'%m/%d/%Y') + timedelta(weeks=week_window)
    return get_time_series_from_range(start_date,end_date,department)
...
def get_time_series_window(
    ...
    start_date = datetime.strptime("10/05/2019",'%m/%d/%Y') + timedelta(weeks=week_window)
    end_date = datetime.strptime("07/09/2022",'%m/%d/%Y') + timedelta(weeks=week_window)
    return get_time_series_from_range(start_date,end_date,department)